...
std::string EncodingConversion::ToString(const std::wstring& wstr)
{
	...
	static const mbs_facet_t& cvt = std::use_facet<mbs_facet_t>(std::locale());
	static std::wstring_convert<mbs_facet_t> converter(&cvt);
...
{
	static std::wstring_convert<mbs_facet_t> conv(new mbs_facet_t(GBK_LOCALE_NAME));
...
{
#if defined(_MSC_VER)
	static std::wstring_convert<std::codecvt_utf8_utf16<uint16_t>, uint16_t> convert;
	auto tmp = convert.from_bytes(str.data(), str.data() + str.size());
...
#if defined(_MSC_VER)
	static std::wstring_convert<std::codecvt_utf16<uint32_t>, uint32_t> convert;
	auto tmp = convert.from_bytes(bytes);
	return std::u32string(tmp.data(), tmp.data() + tmp.size());
#else
	static std::wstring_convert<std::codecvt_utf16<char32_t>, char32_t> convert;
